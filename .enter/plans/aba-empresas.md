...
- **CPF/CNPJ** (obrigatório; **máscara única que alterna automaticamente** entre CPF `000.000.000-00` e CNPJ `00.000.000/0000-00` conforme a quantidade de dígitos; válido com 11 dígitos = CPF ou 14 dígitos = CNPJ)
- **inscrição estadual** (opcional)

Decisões confirmadas: por enquanto o cadastro é **somente no painel de Administração** (sem lista no painel do departamento).
...
create table if not exists public.companies (
  ...
  name text not null,
  documento text not null,          -- CPF (11 dígitos) ou CNPJ (14 dígitos)
...
```
**RLS nas duas tabelas na mesma migração**, com política de admin (`is_admin()`), pois o cadastro é restrito ao administrador:
```sql
...
- `src/lib/types.ts`: `Company { id; numero; name; documento; inscricao_estadual; ufs: string[]; created_at }` e `CompanyWithDepartments extends Company { department_ids: string[] }`.
- `src/lib/mappers.ts`: `toCompanyWithDepartments(row)` (normaliza `company_departments[].department_id`).
- `src/lib/utils.ts`: `formatCpfCnpj(value)` (máscara que alterna CPF/CNPJ) e `isValidCpfCnpj(value)` (11 ou 14 dígitos).
...
- `useUpdateCompany()` — atualiza e substitui os vínculos (apaga + insere).
...
### 5. UI do painel
- Novo `src/components/admin/CompanyFormDialog.tsx`: campos numero, nome, **CPF/CNPJ** (máscara automática + validação), inscrição estadual, UF (múltipla) e departamentos (checkboxes). Valida obrigatórios: numero, nome, UF e CPF/CNPJ.
- Nova `src/pages/admin/CompaniesTab.tsx`: título + botão "Nova empresa", tabela (Numero, Nome, CPF/CNPJ, UF, Departamentos, Ações) com editar/excluir e diálogo de confirmação (padrão de `DepartmentsTab`).
...
**Reuso:** padrão de `DepartmentsTab`/`DepartmentFormDialog` (tabela + diálogo), `Input`/`Checkbox`/`Table`/`AlertDialog`/`Badge`, `useDepartments`, `useQueryClient`.
...
- [ ] Adicionar `Company`/`CompanyWithDepartments` em `types.ts`, `toCompanyWithDepartments` em `mappers.ts`, `formatCpfCnpj`/`isValidCpfCnpj` em `utils.ts` e a lista de UFs.
...
- [ ] Criar `CompanyFormDialog.tsx` com os campos, máscara automática CPF/CNPJ, UF múltipla e checkboxes de departamentos.
...
- [ ] Digitar 11 dígitos no campo CPF/CNPJ → formato `000.000.000-00`; digitar o 12º dígito → muda automaticamente para `00.000.000/0000-00` (até 14 dígitos).
- [ ] Documento com menos de 11 ou entre 12 e 13 dígitos → bloqueia; 11 ou 14 dígitos → salva.
- [ ] Criar empresa com numero, nome, CPF/CNPJ, 2 UFs e 2 departamentos → aparece na tabela; `select * from company_departments` confirma 2 linhas.
- [ ] Sem numero/nome/UF/documento → bloqueia com mensagem de campo obrigatório.
- [ ] Editar empresa trocando UFs e departamentos → tabela e vínculos atualizados.
- [ ] Excluir empresa → some da tabela; vínculos removidos. Excluir um departamento vinculado → vínculo cai por cascade sem afetar as empresas.
