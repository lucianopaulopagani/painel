...
import {
  useAlvaraSanitario,
  useAlvaraSanitarioAll,
  useSaveAlvaraSanitario,
} from "@/hooks/use-alvara-sanitario";
...
import {
  ALVARA_SANITARIO_ENVIADO_OPTIONS,
  ALVARA_SANITARIO_GERADO_OPTIONS,
  alvaraSanitarioAnos,
} from "@/lib/alvara-sanitario";
...
import type { AlvaraSanitarioInput } from "@/lib/types";

const STORAGE_KEY = "societario:alvara-sanitario-ano";
...
export default function AlvaraSanitario() {
  ...
  const { data: records } = useAlvaraSanitario(ano);
  const { data: allRecords } = useAlvaraSanitarioAll();
  const saveMutation = useSaveAlvaraSanitario(ano);
  ...
  const rows = (companies ?? [])
    .filter(
      (company) =>
        companyUsesSubmenu(company, societario?.id, "Alvará Sanitário") &&
    ...
    saveMutation.mutate(
      {
        ...
        ...patch,
      } as unknown as AlvaraSanitarioInput,
      {
        onError: (error) => {
          toast.error(
  ...
  const handleReportExcel = () => {
    downloadAlvaraReportExcel(reportRows, reportFilters, "Alvará Sanitário");
  };
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold">Alvará Sanitário</h2>
          ...
          <Table className="table-fixed min-w-[1240px]">
            <TableHeader>
              <TableRow className="bg-muted hover:bg-muted">
                ...
                {renderFilterSelect("gerado", "Gerado", ALVARA_SANITARIO_GERADO_OPTIONS, "w-24")}
                {renderFilterSelect(
                  "enviado",
                  "Enviado",
                  ALVARA_SANITARIO_ENVIADO_OPTIONS,
                ...
                <TableRow>
                  ...
                  >
                    {Object.values(busca).some(Boolean) && rows.length > 0
                      ...
                      : `Nenhuma empresa com o subdepartamento "Alvará Sanitário" no departamento ${SOCIETARIO_DEPARTMENT_NAME}. Marque-o no cadastro de empresas para que elas apareçam aqui.`}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
